...
--------------------------------------------------------------------------------
local uuid = require 'abel.util.core.uuid'
local M = {}
...
---@class Message
---@field id        string         unique message id (stable for replace_last)
...
local function generate_new_id()
    local base_id = generate_base_id()
    _msg_counters[base_id] = 0

    return base_id .. '-0'
end

local function increment_cover_id(cover_id)
    local high_str, current_version = cover_id:match '^(.-)%-(%d+)$'
    if not high_str then
        return cover_id
    end

    _msg_counters[high_str] = math.max(_msg_counters[high_str] or 0, tonumber(current_version)) + 1

    local version = _msg_counters[high_str]
    if version >= 9999 then
        _msg_counters[high_str] = nil
    end

    return high_str .. '-' .. version
end
...
function M.emit(tag, level, content, data, cover_id)
    ...
    if not ok then
        ...
        Stat.Busy = false
        _bus_log('[Emiter] dispatcher panic for `' .. err .. '`', vim.log.levels.ERROR)
    end
